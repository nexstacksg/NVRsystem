...
This document describes the integration of [go2rtc](https://github.com/AlexxIT/go2rtc) with Oneberry to provide WebRTC streaming capabilities. Note that this is WebRTC (Real-Time Communication) streaming, not WebSocket communication.

## Overview

go2rtc is a lightweight, high-performance WebRTC server that can be used to stream video from various sources, including RTSP cameras. The integration with Oneberry allows for:

- WebRTC streaming of camera feeds with low latency
- Maintaining existing HLS and MP4 recording functionality
- Seamless management of go2rtc alongside the main Oneberry application
...
3. **Stream Integration**: Bridges Oneberry's stream management with go2rtc
...
- Oneberry installed and configured
...
- Create a configuration directory at `/etc/oneberry/go2rtc`
...
### Building Oneberry with go2rtc Support
...
# Specify go2rtc configuration directory (default: /etc/oneberry/go2rtc)
...
When go2rtc integration is enabled, Oneberry will automatically:

1. Start the go2rtc process when needed
2. Register streams with go2rtc when they are added to Oneberry
3. Unregister streams when they are removed from Oneberry
...
These URLs can be used in web applications that support WebRTC, including the Oneberry web interface.

## Configuration

### go2rtc Configuration

The go2rtc configuration file is located at `[GO2RTC_CONFIG_DIR]/go2rtc.yaml`. While Oneberry manages the streams section automatically, you can modify other settings such as:
...
2. Restart the go2rtc process through Oneberry
3. Use go2rtc's API directly for custom configurations

## Health Monitoring

Oneberry includes a health monitoring system for go2rtc:
...
Oneberry uses go2rtc's `frame.jpeg` endpoint for object detection:
...
2. **Streams not appearing in go2rtc**
   ...
   - Check if Oneberry can access the streams directly
   - Look for errors in the Oneberry logs
...
- Oneberry logs contain information about go2rtc integration

## References

- [go2rtc GitHub Repository](https://github.com/AlexxIT/go2rtc)
- [WebRTC Documentation](https://webrtc.org/)
- [Oneberry Documentation](https://github.com/oneberry/oneberry)
