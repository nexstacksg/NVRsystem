 ...
 * MP4 supports AAC, MP3, AC3, and a few other codecs, but NOT PCM variants
 *
 * @param codec_id The codec ID to check
 * @return true if compatible, false otherwise
 */
static bool is_audio_codec_mp4_compatible(enum AVCodecID codec_id) {
    switch (codec_id) {
...
/**
 * Setup AAC codec parameters for output stream when input audio is incompatible
 * This is needed when the input stream uses PCM codecs like G.711 μ-law
 *
 * @param out_codecpar Output codec parameters to set up
 * @param in_codecpar Input codec parameters (for sample rate, channels, etc.)
 * @return 0 on success, negative on error
 */
static int setup_aac_codec_params(AVCodecParameters *out_codecpar, const AVCodecParameters *in_codecpar) {
    const AVCodec *encoder = avcodec_find_encoder(AV_CODEC_ID_AAC);
    if (!encoder) {
        log_error("AAC encoder not found - cannot setup audio for MP4");
        return -1;
    }

    AVCodecContext *encoder_ctx = avcodec_alloc_context3(encoder);
    if (!encoder_ctx) {
        log_error("Failed to allocate AAC encoder context");
        return -1;
    }

    // Set encoder parameters based on input stream
    encoder_ctx->sample_fmt = AV_SAMPLE_FMT_FLTP; // AAC requires float planar format
    encoder_ctx->sample_rate = in_codecpar->sample_rate > 0 ? in_codecpar->sample_rate : 8000;
    encoder_ctx->bit_rate = 128000; // 128 kbps

    // Handle channel layout for different FFmpeg versions
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(59, 37, 100)
    // For FFmpeg 5.0+
    if (in_codecpar->ch_layout.nb_channels > 0) {
        av_channel_layout_copy(&encoder_ctx->ch_layout, &in_codecpar->ch_layout);
    } else {
        av_channel_layout_default(&encoder_ctx->ch_layout, 1); // Default to mono for G.711
    }
#else
    // For older FFmpeg versions
    encoder_ctx->channels = in_codecpar->channels > 0 ? in_codecpar->channels : 1;
    ...
    // Open encoder to initialize it properly
    int ret = avcodec_open2(encoder_ctx, encoder, NULL);
    if (ret < 0) {
        log_error("Failed to open AAC encoder for setup: %d", ret);
        avcodec_free_context(&encoder_ctx);
        return ret;
    }

    // Copy encoder parameters to output codec parameters
    ret = avcodec_parameters_from_context(out_codecpar, encoder_ctx);
    if (ret < 0) {
        log_error("Failed to copy AAC parameters to output: %d", ret);
        avcodec_free_context(&encoder_ctx);
        return ret;
    }

    log_info("Set up AAC codec parameters for MP4 output (sample_rate=%d)", encoder_ctx->sample_rate);

    avcodec_free_context(&encoder_ctx);
    return 0;
}

// Forward declaration of transcode_audio_packet function from mp4_writer_utils.c
...
int record_segment(const char *rtsp_url, const char *output_file, int duration, int has_audio,
    ...
    // Flag to track whether audio needs to be transcoded (PCM to AAC)
    bool audio_needs_transcoding = false;
    ...
    if (audio_stream_idx >= 0 && has_audio) {
        AVCodecParameters *input_audio_codecpar = input_ctx->streams[audio_stream_idx]->codecpar;
        
        // Check if the input audio codec is compatible with MP4 format
        if (is_audio_codec_mp4_compatible(input_audio_codecpar->codec_id)) {
            // Compatible codec - just copy the parameters
            log_info("Including audio stream in MP4 recording (codec: %s)", 
                    avcodec_get_name(input_audio_codecpar->codec_id));
            ...
            ret = avcodec_parameters_copy(out_audio_stream->codecpar, input_audio_codecpar);
            if (ret < 0) {
            ...
            // Incompatible codec (like G.711 μ-law) - transcode to AAC
            log_info("Input audio codec '%s' is not MP4 compatible - will transcode to AAC",
                    avcodec_get_name(input_audio_codecpar->codec_id));
            
            out_audio_stream = avformat_new_stream(output_ctx, NULL);
            if (!out_audio_stream) {
                log_error("Failed to create output audio stream for transcoding");
                // Fall back to recording without audio
                has_audio = 0;
            } else {
                // Set up AAC codec parameters for the output stream
                int aac_ret = setup_aac_codec_params(out_audio_stream->codecpar, input_audio_codecpar);
                if (aac_ret < 0) {
                    log_error("Failed to set up AAC codec parameters (error: %d), recording without audio", aac_ret);
                    // Remove the audio stream we just added by nulling it
                    out_audio_stream = NULL;
                    has_audio = 0;
                } else {
                    out_audio_stream->time_base = (AVRational){1, out_audio_stream->codecpar->sample_rate};
                    audio_needs_transcoding = true;
                    log_info("Audio transcoding enabled: %s -> AAC",
                            avcodec_get_name(input_audio_codecpar->codec_id));
                }
            }
        }
    ...
    while (1) {
        ...
        // Process audio packets - only if audio is enabled and we have an audio output stream
            ...
            // If transcoding is needed, transcode the PCM packet to AAC
            if (audio_needs_transcoding) {
                ...
                // Transcode the audio packet from PCM to AAC
                int transcode_ret = transcode_audio_packet(
                    rtsp_url ? rtsp_url : "unknown",
                    pkt,
                    transcoded_pkt,
                    input_ctx->streams[audio_stream_idx]);

                if (transcode_ret < 0) {
                    // Transcoding failed, skip this packet
                    log_debug("Failed to transcode audio packet, skipping");
                ...
                // Set the output stream index for the transcoded packet
                ...
                // Write packet directly (no transcoding needed)
                ret = av_interleaved_write_frame(output_ctx, pkt);
            }
            if (ret < 0) {
                char error_buf[AV_ERROR_MAX_STRING_SIZE] = {0};
