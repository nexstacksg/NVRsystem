...
static int init_audio_transcoder(const char *stream_name,
    ...
    // Set up SwrContext for sample format conversion (S16 -> FLTP)
    // G.711 μ-law decodes to S16, but AAC requires FLTP
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(59, 37, 100)
    ...
    // Allocate resampled frame for the converted audio (FLTP format for AAC)
    ...
    audio_transcoders[slot].resampled_frame->format = audio_transcoders[slot].encoder_ctx->sample_fmt;
    audio_transcoders[slot].resampled_frame->sample_rate = audio_transcoders[slot].encoder_ctx->sample_rate;
#if LIBAVCODEC_VERSION_INT >= AV_VERSION_INT(59, 37, 100)
    av_channel_layout_copy(&audio_transcoders[slot].resampled_frame->ch_layout,
                           &audio_transcoders[slot].encoder_ctx->ch_layout);
#else
    audio_transcoders[slot].resampled_frame->channel_layout = audio_transcoders[slot].encoder_ctx->channel_layout;
    audio_transcoders[slot].resampled_frame->channels = audio_transcoders[slot].encoder_ctx->channels;
#endif
    // Set frame size to encoder's expected frame size (typically 1024 for AAC)
    if (audio_transcoders[slot].encoder_ctx->frame_size > 0) {
        audio_transcoders[slot].resampled_frame->nb_samples = audio_transcoders[slot].encoder_ctx->frame_size;
    } else {
        audio_transcoders[slot].resampled_frame->nb_samples = 1024;
    }
    ret = av_frame_get_buffer(audio_transcoders[slot].resampled_frame, 0);
    if (ret < 0) {
...
int transcode_audio_packet(const char *stream_name,
    ...
    // Receive frame from decoder (S16 format for G.711 μ-law)
    ...
    // Convert sample format from S16 (decoder output) to FLTP (AAC encoder input)
    // using SwrContext - this is essential for audio transcoding to work
    AVFrame *resampled = audio_transcoders[transcoder_idx].resampled_frame;
    av_frame_make_writable(resampled);
    resampled->nb_samples = audio_transcoders[transcoder_idx].encoder_ctx->frame_size > 0
                          ? audio_transcoders[transcoder_idx].encoder_ctx->frame_size : 1024;
    ...
    if (ret < 0) {
        log_ffmpeg_error(ret, "Failed to convert audio sample format");
        return ret;
    }
    resampled->nb_samples = ret;

    // Copy timestamp info from decoded frame to resampled frame
    resampled->pts = audio_transcoders[transcoder_idx].frame->pts;

    // Send resampled frame (FLTP format) to AAC encoder
    ret = avcodec_send_frame(audio_transcoders[transcoder_idx].encoder_ctx,
                            resampled);
    if (ret < 0) {
        log_ffmpeg_error(ret, "Failed to send resampled frame to encoder");
        return ret;
    }
